...
# --------- BCT training
def train(samples, labels, max_tree_depth = 5, max_tree_count = 100, minTPR = 0.99, maxFPR = 0.01):
    """Trains a boosted classification tree ensamble.

    Args:
        samples (matrix): Rows of arrays (samples).
        labels (array): Array of labels: zeros and ones.
        max_tree_depth (int, optional): Target tree depth. Defaults to 5.
        max_tree_count (int, optional): Target tree count. Defaults to 15.
        minTPR (float, optional): Minimal TPR to satisfy.
        maxFPR (float, optional): Maximum FPR.

    Returns:
        BCT: Boosted classification trees.
    """
    
    tree_cnt = 0; FPR = 1.0; threshold = -1000
    confs = np.ones(len(samples)) / len(samples)
    bct = BCT()
